...
启用 Web API 并预先配置管理 API Token 后，打开 Web 管理页并输入该 Token 登录控制台。首次使用按“产品配置”页的三步提示完成即可：基础配置只保留运行 Bot 所需的 Telegram 配置和一般无需修改的界面语言；Userbot、运行性能、下载工具与 Web API 位于默认折叠的“额外功能与高级设置”中，需要时再配置。
...
<br />
请务必使用自己的 API ID 与 API Hash，且不要向任何人泄露验证码或 2FA 密码.
{{< /hint >}}

首次登录可以直接在 Web 管理页完成。Userbot 卡片会依次引导保存 App ID、App Hash 与会话文件路径、登录个人账号、启用并重启；监听地址和管理 API Token 未改变时，页面只会在检测到新服务实例后自动重新连接。页面会显示当前 Bot 与 Userbot 账号，已连接的 Userbot 可通过“退出并切换账号”停用、注销会话并重启。验证码与密码不会写入配置。

也可以使用交互式终端登录；Docker 部署需使用 `-it` 运行一次登录流程。成功生成会话文件后，后续启动不再需要重复输入。
