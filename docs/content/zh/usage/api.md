...
API 启动后，访问 `http://<服务器地址>:<端口>/` 即可打开内置控制台。控制台提供产品配置、任务管理、Bot Relay、运行日志和高级配置页面；常用选项可通过表单配置，完整 `config.toml` 仍可在高级配置中编辑。
...
### GET /api/v1/logs — 读取运行日志

按日期读取最新日志。`date` 使用 `YYYY-MM-DD`，默认读取最近的日志日期；`level` 可选 `debug`、`info`、`warn`、`error`、`fatal`，返回该级别及以上的日志，默认为 `debug`；`limit` 默认为 500，最大为 2000。响应中的 `dates` 按日期倒序，`lines` 按时间倒序。
...
返回 HTTP API、Telegram Bot 和 Bot Relay 创建的保存任务，并按创建时间由新到旧排序。记录仅保存在内存中，重启后清空；已进入终态的任务还会在最后一次更新 24 小时后清理，清理任务每 10 分钟执行一次。`source` 为 `api`、`bot` 或 `relay`。
